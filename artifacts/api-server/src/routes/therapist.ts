...
// Removed: the router.use("/therapist", ...) prefix gate that used to
// live here. Traced (chat, vertical-visibility follow-up) and confirmed
// unsafe — "/therapist" itself covers far more than new request-creation:
// my-candidacies, respond-request, confirm-interview, accept-trial,
// mark-assessment-done, and every /therapist/engagements/* professional-
// management route all matched this prefix too, contradicting admin.tsx's
// own "already-in-progress work is unaffected" claim. Replaced with a
// single inline check inside POST /therapist/request only (the one actual
...
router.post("/therapist/request", requireAuth, requireRole("parent"), async (req: Request, res: Response): Promise<void> => {
  ...
  const [visibilitySettings] = await db
    .select({ therapistVisible: adminSettingsTable.therapistVisible })
    .from(adminSettingsTable)
    .limit(1);
  if (visibilitySettings?.therapistVisible === false) {
